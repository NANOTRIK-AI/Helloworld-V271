...
describe("V271Token", function () {
  ...
  beforeEach(async function () {
    ...
    // Pass {} as overrides to ensure the parameter is correctly interpreted.
  ...
  describe("Staking", function () {
    it("Should allow staking and calculate correct yield", async function () {
      ...
      // Stake tokens and wait for confirmation
      const stakeTx = await v271.connect(addr1).stake(stakeAmount, BigInt(stakeDuration));
      await stakeTx.wait();
