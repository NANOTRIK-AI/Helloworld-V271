...
describe("V271Token", function () {
  ...
  const APERY_CONSTANT = BigInt("1202056903159594285"); // ζ(3) * 10^18
  const INITIAL_SUPPLY = BigInt(271000000) * BigInt(10)**BigInt(18); // 271 million tokens
  ...
  beforeEach(async function () {
    ...
    await v271Token.waitForDeployment();
  });

  describe("Deployment", function () {
    it("Should set the right owner with correct roles", async function () {
      // Check if owner has DEFAULT_ADMIN_ROLE
      const DEFAULT_ADMIN_ROLE = await ethers.zeroPadValue("0x00", 32);
      const PAUSER_ROLE = ethers.keccak256(ethers.toUtf8Bytes("PAUSER_ROLE"));
      const MINTER_ROLE = ethers.keccak256(ethers.toUtf8Bytes("MINTER_ROLE"));
      const ORACLE_ROLE = ethers.keccak256(ethers.toUtf8Bytes("ORACLE_ROLE"));

      expect(await v271Token.hasRole(DEFAULT_ADMIN_ROLE, owner.address)).to.equal(true);
      expect(await v271Token.hasRole(PAUSER_ROLE, owner.address)).to.equal(true);
      expect(await v271Token.hasRole(MINTER_ROLE, owner.address)).to.equal(true);
      expect(await v271Token.hasRole(ORACLE_ROLE, owner.address)).to.equal(true);
    });

    it("Should assign the total supply of tokens to the owner", async function () {
      const ownerBalance = await v271Token.balanceOf(owner.address);
      expect(await v271Token.totalSupply()).to.equal(ownerBalance);
  ...
  describe("Mathematical constants", function () {
    it("Should have the correct value for Apéry's constant", async function () {
      expect(await v271Token.APERY_CONSTANT()).to.equal(APERY_CONSTANT);
    });
  });
  ...
  describe("Transactions", function () {
    it("Should transfer tokens between accounts", async function () {
      ...
      const secondTransferAmount = ethers.parseEther("25");
      await v271Token.connect(addr1).transfer(addr2.address, secondTransferAmount);
      
      // Check final balances
      expect(await v271Token.balanceOf(addr1.address)).to.equal(transferAmount - secondTransferAmount);
      expect(await v271Token.balanceOf(addr2.address)).to.equal(secondTransferAmount);
    });

    it("Should fail if sender doesn't have enough tokens", async function () {
      const initialOwnerBalance = await v271Token.balanceOf(owner.address);
      
      // Try to send more tokens than the owner has
      await expect(
        v271Token.connect(addr1).transfer(owner.address, 1)
      ).to.be.revertedWith("ERC20: transfer amount exceeds balance");

      // Owner balance shouldn't have changed
      expect(await v271Token.balanceOf(owner.address)).to.equal(initialOwnerBalance);
    });
  });

  describe("Conditional transfer", function () {
    it("Should transfer tokens when condition is met", async function () {
      // Set up condition values where zetaWeighted exceeds threshold
      // For example, if condition = 10^18 and threshold = 10^18,
      // then zetaWeighted = 1202056903159594285 which is > 10^18
      const transferAmount = ethers.parseEther("100");
      const condition = ethers.parseEther("1"); // 1 with 18 decimals
      const threshold = ethers.parseEther("1"); // 1 with 18 decimals
      
      // Perform conditional transfer
      await v271Token.conditionalTransfer(
        addr1.address,
        transferAmount,
        condition,
        threshold
    ...
    it("Should fail when condition is not met", async function () {
      // Set up condition values where zetaWeighted is less than threshold
      const transferAmount = ethers.parseEther("100");
      const condition = ethers.parseEther("1"); // 1 with 18 decimals
      const threshold = ethers.parseEther("2"); // 2 with 18 decimals (which will be > zetaWeighted)
