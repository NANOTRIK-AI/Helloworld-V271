...
describe("V271Token", function () {
  ...
  beforeEach(async function () {
    ...
    v271 = await V271Token.deploy(ethers.parseEther("1000000000000000000000000")); // Providing initial supply parameter
  });
  ...
  describe("Staking", function () {
    it("Should allow staking and calculate correct yield", async function () {
      ...
      // Stake tokens (no approval needed if using _transfer internally)
      await v271.connect(addr1).stake(stakeAmount, stakeDuration);
      
      // Get stake information using the implemented getStake function
      const stake = await v271.connect(addr1).getStake() as { amount: bigint, duration: bigint };
